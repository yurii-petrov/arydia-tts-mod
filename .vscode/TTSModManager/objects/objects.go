...
func (d *db) print(l file.TextReader, x file.TextReader, order []string) (ObjArray, error) {
    ...
    fmt.Printf("❗ Mismatch detected: expected order (%d) and db.root (%d) to have the same length\n", len(order), len(d.root))

    // Create a map from the order list for quick lookup
    orderMap := make(map[string]bool)
    for _, id := range order {
        orderMap[id] = true
    }

    fmt.Println("🔸 Objects found in db.root but missing in order list:")
    for guid := range d.root {
        if !orderMap[guid] {
            fmt.Printf(" - %s\n", guid)
        }
    }

    fmt.Println("🔹 GUIDs listed in order but not found in db.root:")
    for _, guid := range order {
        if _, ok := d.root[guid]; !ok {
            fmt.Printf(" - %s\n", guid)
        }
    }

    return nil, fmt.Errorf("expected order (%d) and db.root (%d) to have same length", len(order), len(d.root))
}
