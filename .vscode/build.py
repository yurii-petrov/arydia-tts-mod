...
def main():
    ...
    if args.action == "build":
        ...
        if os.name == "nt":
            # Windows: PyGetWindow has getWindowsWithTitle
            for window in pygetwindow.getWindowsWithTitle(target_title):
                if window.title == target_title:
                    ...
                    time.sleep(0.5)
                    pyautogui.press(HOTKEY)  # press single key (F13)
                    break

        else:
            # macOS: use getAllTitles + activate(title)
            titles = pygetwindow.getAllTitles()
            match = next((t for t in titles if (t or "") == target_title), None)

            # If exact match not found, try "contains" (macOS titles can vary)
            if not match:
                match = next(
                    (t for t in titles if target_title.lower() in (t or "").lower()),
                    None
                )

            if match:
                try:
                    pygetwindow.activate(match)
                except Exception as e:
                    print(f"[warn] macOS: failed to activate window '{match}': {e}")

                time.sleep(0.5)
                pyautogui.press(HOTKEY)
            else:
                print(f"[warn] macOS: Tabletop Simulator window not found. Titles: {titles}")
